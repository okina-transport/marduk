...
class RARToZipFilesSplitterTest {

    @Test
    void testSplitSimpleRarFile() throws IOException {
        checkSplitParts("src/test/resources/NRI 20160219.rar", 15);
    }

    @Test
    void testSplitComplexRarFile() throws IOException {
        checkSplitParts("src/test/resources/NRI 20160426.rar", 21);
    }

    @Test
    void testSplitComplexRarFileWithError() throws IOException {
        checkSplitParts("src/test/resources/NRI 20160603.rar", 2);
    }

    private void checkSplitParts(String filename, int numParts) throws IOException {
        CamelContext ctx = new DefaultCamelContext();
        Exchange ex =  ExchangeBuilder.anExchange(ctx).build();
