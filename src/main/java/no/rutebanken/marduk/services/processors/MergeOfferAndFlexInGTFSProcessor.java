...
public class MergeOfferAndFlexInGTFSProcessor implements Processor {
    ...
    private void generateNewZip(Exchange exchange) throws IOException {
        ...
        if (StringUtils.isBlank(offerFileName) && StringUtils.isNotBlank(exchange.getIn().getHeader(EXPORT_NAME, String.class))){
            String exportName = exchange.getIn().getHeader(EXPORT_NAME, String.class);
            offerFileName = exportName.endsWith(".zip") ? exportName : exportName + ".zip";
        }
