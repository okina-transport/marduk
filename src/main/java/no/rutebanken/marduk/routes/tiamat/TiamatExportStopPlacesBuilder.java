...
import no.rutebanken.marduk.routes.chouette.NotifyExportTemplateJobCompletionProcessor;
...
public class TiamatExportStopPlacesBuilder extends AbstractChouetteRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from(TIAMAT_EXPORT_ROUTING_DESTINATION).streamCache(Boolean.TRUE)
                ...
                .process(e -> {
                    if (e.getIn().getBody() instanceof Job) {
                        e.getIn().setHeader(ORIGINAL_JOB, e.getIn().getBody());
                    }
        ...
        from("direct:processTiamatExportEnd")
            ...
            .process(notifyExportTemplateJobCompletionProcessor)
            .choice()
                .when(header(CHOUETTE_REFERENTIAL).isEqualTo("technique"))
