...
import no.rutebanken.marduk.beans.Md5ChecksumExtractorBean;
import no.rutebanken.marduk.aggregation.Md5AggregationStrategy;
...
public class MapDataRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        //TODO Can this process be simplified, using e.g. direct calls?

        from("quartz2://backend/mapdata?cron=0+0/10+*+*+*+?")   //TODO configure outside code
        ...
        from("file:target/files/input/mapdata?readLock=changed")
            .log("Got new file with name: ${header.CamelFileNameOnly}")
                .setHeader("md5").method(Md5ChecksumExtractorBean.class)
                    .log("Set md5 header from ${header.CamelFileNameOnly} file to: ${header.md5}")
                    .aggregate(header("md5"), new Md5AggregationStrategy())
                    .completionSize(2)
                    .completionTimeout(5 * 60 * 1000)  //TODO configure together with quartz trigger
